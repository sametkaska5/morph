...
export async function addPhotosToEntry({ userId, entryId, uris }: AddPhotosPayload) {
  ...
  let order = nextOrderIndex(existing ?? []);

  for (const uri of uris) {
    const { base64, thumbBase64 } = await resizeAndCompress(uri);
    const storagePath = await uploadPhoto(userId, entryId, base64);

    // Thumbnail üretimi başarısız olsa bile fotoğrafı kaybetmiyoruz: thumb_path
    // null kalır, okuyan taraf storage_path'e düşer (bkz. migration 0009).
    ...
    const { error: insertError } = await supabase
      .from("photos")
      .insert({ entry_id: entryId, storage_path: storagePath, thumb_path: thumbPath, order_index: order });
